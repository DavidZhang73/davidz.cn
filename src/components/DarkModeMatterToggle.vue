<template>
  <div>
    ...
    <svg
        ...
        class="absolute w-full inset-x-0 top-[-20px] mx-auto h-10 w-10 text-white dark:text-gray-300 cursor-pointer"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
...
onMounted(() => {
  ...
  const cord = Composites.stack(250, 100, 1, 15, 0, 0, function (x: number, y: number) {
  ...
  canvas.value.querySelector('canvas').addEventListener('mousemove', handleMouseMove)
  canvas.value.querySelector('canvas').addEventListener('touchmove', handleTouchMove)
  Events.on(engine, 'collisionStart', (event: IEventCollision<EngineType>) => {
    const pairs = event.pairs
    for (let i = 0, j = pairs.length; i != j; ++i) {
      const pair = pairs[i]
      if (mouseEvent)
        canvas.value.querySelector('canvas').dispatchEvent(new MouseEvent('mouseup', mouseEvent))
      else if (touchEvent)
        canvas.value.querySelector('canvas').dispatchEvent(new TouchEvent('touchend', {
          // @ts-ignore
